...
public class File
{
	...
	{
		return new ByteArrayInputStream( m_parent.m_binArray, m_startData, m_size );
	}

	public void fileToField( ResultSet rs, String columnName ) throws SQLException, SmartUploadException, IOException,
		ServletException
	{
		long numBlocks = 0L;
		...
		try
		{
			...
			{
				rs.updateBinaryStream( columnName, new ByteArrayInputStream( m_parent.m_binArray, pos, blockSize ),
									   blockSize );
				pos = pos != 0 ? pos : 1;
				pos = i * blockSize;
			}
			...
			if ( leftOver > 0 )
			{
				rs.updateBinaryStream( columnName, new ByteArrayInputStream( m_parent.m_binArray, pos, leftOver ),
									   leftOver );
			}
		}
