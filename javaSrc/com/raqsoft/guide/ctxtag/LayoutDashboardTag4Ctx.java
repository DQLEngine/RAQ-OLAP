...
public class LayoutDashboardTag4Ctx extends TagSupport{
	...
	private String tempDBName;//tag
	private String resetLmd;//tag
	
	public String getResetLmd() {
		return resetLmd;
	}

	public void setResetLmd(String resetLmd) {
		this.resetLmd = resetLmd;
	}
	...
	private void initParameters() {
		...
		showHistoryRpx=getDefaultParam( showHistoryRpx, "yes" );
		tempDBName=getDefaultParam( tempDBName, "raq_tempDB");
		resetLmd=getDefaultParam( resetLmd, "yes");
	}
...
//				}
			...
			if(olapStr.length() >0){
				String olapJsonStr = olapStr.replace("<d_q>", "\"");
				JSONObject jo = new JSONObject(olapJsonStr);
				JSONArray dataSetsJa = (JSONArray) jo.get("dataSets");
				JSONObject ds1 = (JSONObject) dataSetsJa.get(0);
				this.dataSource = (String) ds1.get("dataSource");
				this.tempDBName = (String) ds1.get("dataSource");
			}
			String json = "";
			...
			boolean resetLmdFlag = true;
			resetLmdFlag = "yes".equals(resetLmd);
			...
			if (isGlmd) {
				if(dataSource == null || dataSource.length() == 0){
					...
					if(glmdPath != null && glmdPath.length() != 0){
						dataSource = this.tempDBName;
						DBUtil.putGlmdIntoDBList(glmdPath, dataSource, configFile);
					}else{
						//throw new Exception("no glmd file or dataSource");
						noGlmdOrDs=true;
					}
				}
				if(resetLmdFlag) json = ConfigUtil.getListJsonData(dataSource);
				else json = "";
			}else{
...
//			out.println("guideConf.fileHome = '"+fileHome+"';");
			...
			pageContext.include(guideDir+"jsp/dashboardInclude_c.jsp?jsv=new&guideDir="+URLEncoder.encode(guideDir, "UTF-8")+"&title="+URLEncoder.encode(title, "UTF-8"),true);
