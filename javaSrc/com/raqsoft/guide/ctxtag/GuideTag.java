...
//			}
			...
			if (dataSource.length()>0 && DataSphereServlet.DQLDATASOURCES.indexOf(dataSource)>=0) { //"source".equalsIgnoreCase(view)
				try {
					if(isGlmd) {
						...
						json = ConfigUtil.getListJsonData(dataSource);
					}
					else json = ConfigUtil.getMetaDataJson(dataSource);
				} catch (Throwable e) {
					//e.printStackTrace();
			...
			pageContext.include(guideDir+"jsp/template_c.jsp?jsv=v1&guideDir="+URLEncoder.encode(guideDir, "UTF-8")+"&title="+URLEncoder.encode(title, "UTF-8"),true);
