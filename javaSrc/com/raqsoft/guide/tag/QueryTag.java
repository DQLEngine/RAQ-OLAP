...
public class QueryTag extends Tag {
	...
	private void initParameters() {
		...
		qyxFolderOnServer = getDefaultParam( qyxFolderOnServer, DataSphereServlet.qyxFolderOnServer );
		...
		if (guideDir.startsWith("/")) guideDir = guideDir.substring(1);
	  	...
	  	dimDataOnServer = getDefaultParam( dimDataOnServer, DataSphereServlet.dimDataFolderOnServer+dimDataName );
...
//			Logger.debug("qyxStr : " + qyxStr);
			...
			String[] infos = getDqlInfo(dataSource,this.dictionary,this.visibility);
			String guideDir = ReportConfig.raqsoftDir;
...
//			out.println("guideConf.view = '"+view+"';");
			...
			pageContext.include(guideDir+"jsp/template.jsp?jsv=new&guideDir="+URLEncoder.encode(guideDir, "UTF-8")+"&title="+URLEncoder.encode(title, "UTF-8"),false);
	...
	public static String[] getDqlInfo(String dataSource, String dct, String vsb) {
		String [] r = new String[3];

		if (dataSource.length()>0) { //"source".equalsIgnoreCase(view)
			try {
		...
		if (dct.length()>0) {
			try {
				r[1] = FileUtils.getDict(DataSphereServlet.getFilePath(dct)).replaceAll("\"", "<d_q>");
			} catch (Exception e) {
		...
		if (vsb.length()>0) {
			try {
				r[2] = FileUtils.getVsb(DataSphereServlet.getFilePath(vsb)).replaceAll("\"", "<d_q>");
			} catch (Exception e) {
				Logger.warn("read visibility error ! " + vsb);
				r[2] = "";
			}
		} else r[2] = "";
		return r;
	...
	public static String getFiles() {
		...
		if (DataSphereServlet.dfxFolderOnServer != null ) {
			List<String> dfxs = new ArrayList<String>();
			String dfxMain = (DataSphereServlet.getFilePath(DataSphereServlet.dfxFolderOnServer)).replaceAll("\\\\", "/");
		...
		if (DataSphereServlet.olapFolderOnServer != null) {
			...
			String olapMain = (DataSphereServlet.getFilePath(DataSphereServlet.olapFolderOnServer)).replaceAll("\\\\", "/");
		...
		if (DataSphereServlet.qyxFolderOnServer != null) {
			...
			String qyxMain = (DataSphereServlet.getFilePath(DataSphereServlet.qyxFolderOnServer)).replaceAll("\\\\", "/");
		...
		if (DataSphereServlet.rpxFolderOnServer != null) {
			...
			String rpxMain = (DataSphereServlet.getFilePath(DataSphereServlet.rpxFolderOnServer)).replaceAll("\\\\", "/");
		...
		if (DataSphereServlet.inputFileFolderOnServer != null) {
			...
			String root = (DataSphereServlet.getFilePath(DataSphereServlet.inputFileFolderOnServer)).replaceAll("\\\\", "/");
			root = root.replaceAll("////", "/").replaceAll("///", "/").replaceAll("//", "/");
			FileUtils.getFileList(inputs, root, new String[]{".bin",".json"});
			String inputs2 = null;
		...
		if (DataSphereServlet.fileDataFolderOnServer != null) {
			List<String> inputs = new ArrayList<String>();
			String root = (DataSphereServlet.getFilePath(DataSphereServlet.fileDataFolderOnServer)).replaceAll("\\\\", "/");
			root = root.replaceAll("////", "/").replaceAll("///", "/").replaceAll("//", "/");
			Logger.debug("getfiles: fileDataFolder is:");
			Logger.debug(root);
			FileUtils.getFileList(inputs, root, new String[]{".txt",".csv",".xls",".xlsx",".btx"});
			String inputs2 = null;
			for (int i=0; i<inputs.size(); i++) {
				if (inputs2 == null) inputs2 = "'" + inputs.get(i).substring(realHome.length()+1) + "'";
				else inputs2 += ",'" + inputs.get(i).substring(realHome.length()+1) + "'";
