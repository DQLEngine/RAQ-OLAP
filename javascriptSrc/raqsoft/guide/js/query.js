...
function loadSub(node){
	...
	if (classTable == null || classTable == '') classTable = guideConf.className;
	if (classTable == null) classTable = '';
	guideConf.className = classTable;
...
function loadTreeEditStyles(){
	...
	var doReload = guideConf.autoReloadDimDataOnServer;
	if((r == null || r == '') && doReload == 'yes') doReload = 'no';//tree有值时候：no不刷新，yes刷新；tree无值都不刷新直接设置成no
	$.ajax({
		type: 'POST',
		async : false,
		url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
		data: {action:2,oper:'generateGuideTrees',dataSource:guideConf.dqlDataSources.indexOf(guideConf.dataSource)>=0?guideConf.dataSource:"",trees:r,file:guideConf.dimDataOnServer,maxDimSize:guideConf.maxDimSize,autoReload:doReload},
		success: function(data){
			if (data.indexOf('error:')==0) {
				alert(data.substring(6));
				return;
...
//		});
		...
		if (olapStr != '') {
			...
			for (var ii = 0; ii<rqAnalyse.rpxs.length; ii++) {
				if (!rqAnalyse.rpxs[ii].reportId) rqAnalyse.rpxs[ii].reportId = "rid"+ suffix+(num++)+Math.floor(Math.random()*10);
					...
					,dataSets:[
						{
							...
							,rowCount:0
							,selectInputFileTableName:guideConf.inputFileTableName
						}
					],rpxs:[
						{
							name:resources.guide.js75
		...
		for (var i=0; i<rqAnalyse.dataSets.length; i++){
			...
			if (dsi.type == 6) {
				...
				var ft = function(dsi) {
					var fileExist = false;
					if (dsi.dataId == '') dsi.dataId = guideConf.dataFolderOnServer+"q"+new Date().getTime()+(guideConf.dataFileType.toLowerCase()=="binary"?".btx":".txt");
					else {
						$.ajax({
							type: 'POST',
							async : false,
...
function manageDataSet(){
	...
	var dsClick = function(){
		...
		$('#inputFiles').on('blur',dsInputFilesChange);
		...
		if (ds.selectInputFileTableName) {
			if(ds.inputFiles) dsInputFilesChange();
			else{
				add_SelDom_insideFileTable([ds.selectInputFileTableName]);
				$('#selDom_insideFileTable').val(ds.selectInputFileTableName);
			}
		}
		...
		var saveFunc = function(silence){
			...
			if (type == 2) {
				...
				var var2 = $('#selDom_insideFileTable').val();
				ds.inputFiles = var1;
				ds.selectInputFileTableName = var2;
			} else if (type == 6) {
	...
	addBut.click(function(){
		...
		rqAnalyse.dataSets.push({
			...
			,tableName:''
			,dqlConf:{}
			,dataId:guideConf.dataFolderOnServer+'q'+new Date().getTime()+"."+(guideConf.dataFileType=='text'?'txt':'btx')//缓存的数据ID
