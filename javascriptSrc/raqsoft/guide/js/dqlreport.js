...
var aly = {
	...
	,queryDataSet : function(reQuery,dataSet,callback) {
		...
		olapObj.server({cursorSize:guideConf.cursorSize, dqlTableFilterName:guideConf.filter,action:2,oper:'query',reQuery:reQuery,dataId:ds.dataId,type:ds.type,dataSource:ds.dataSource,ql:ds.ql,dfxFile:ds.dfxFile,dfxScript:ds.dfxScript,dfxParams:ds.dfxParams,inputFiles:ds.inputFiles,filter:'',currTable:ds.currTable,dataFileType:guideConf.dataFileType,tableName:ds.selectInputFileTableName},function(data){
			if (data.indexOf('error:')==0) {
				alert(data.substring(6));
	...
	,generateReport : function(conf, drillRpx, zongji) {
		...
		var templateRpx = function() {
			...
			var dsj = aly.getDataSet(conf.dataSet);
			var dsj_fields;
			if(dsj.type < 6 && dsj.type > 0){
				dsj_fields = dsj.fields;
				dsj_fields = dsj_fields.concat(conf.fields);
			}else if(dsj.type >= 6){
				dsj_fields = conf.fields;
				dsj_fields = dsj_fields.concat(conf.tops);
				dsj_fields = dsj_fields.concat(conf.lefts);
			}
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {dsj_fields:JSON.stringify(dsj_fields),action:2,oper:'calcReport',reportId:conf.reportId,title:conf.name,reportType:conf.type,fields:fields,template:conf.template,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer},
		...
		var olapRpx = function() {
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {oper:'genReport',dsj_fields:JSON.stringify(dsj_fields),action:2,dataId:aly.getDataSet(conf.dataSet).dataId,zongji:zongji?true:false,infi:guideConf.infinity,reportId:conf.reportId,title:conf.name,rpt:JSON.stringify(rpt).replaceAll('"','<d_q>'),lefts:rpt.lefts,tops:rpt.tops,maxSize:guideConf.maxReportSize,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer,onlyRefreshDs:guideConf.onlyRefreshDs},
				//confs:[{type:1自定义分析报表/2模板报表,name:'报表名称',reportId:'',template:'',show:0/1,isRowData:1,lefts:[{name:'',item:itemId,use:1,order:0无序/1升序/2降序}],tops:[],fields:[],wheres:[{item:itemId}]}...] 调序、排序、改名
				success: success
			});
		}
    	...
    	,button: [
	         {
	             ...
	             callback: function() {
			    	...
			    	if ($('#saveGrpxChk3')[0].checked) {
		 				var path = guideConf.dataFolderOnServer+"/"+$.trim($('#saveGrpxPath2').val())+"/"+name+(guideConf.dataFileType.toLowerCase()=="binary"?".btx":".txt");
				    	$.ajax({
				    		type: 'POST',
				    		async : false,
				    		url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
