		...
		if (dct != null) {
			...
			for (var i=0; i<dct.dimItemList.length; i++) {
				...
				try {
					...
					console.log(e);
					console.log(di.codeCol + " / " + di.dispCol + " / " + e);
				}
			}
			if (dct.classItemList != null && dct.classItemList.length>0) {
				...
				for (var i=0; i<dct.classItemList.length; i++) {
					...
					if (ci.name == guideConf.className) {
						
						for (var z=lmd.dims.length-1; z>=0; z--) {
							if (c.dims.indexOf(lmd.dims[z].name) == -1 && c.dims.indexOf(lmd.dims[z].dispName) == -1) {
								lmd.dims.remove(lmd.dims[z]);
							}
						}
					}
				}
	...
	var mdUtils = {
		...
		getDim : function(dim) {
			...
			if (dim.indexOf('.') > 0) {
				...
				for (var i=0; i<lmd.dims.length; i++) {
					if (ds[0] == lmd.dims[i].table && ds[1] == lmd.dims[i].field) return lmd.dims[i];
				}
	...
	var obj2 = {
		...
		getEditExp4Rpx : function(edit,isRealNum) {
			...
			for(var i=0; i<edit.data.length; i++) {
				...
				codes += edit.data[i].real;
				disps += edit.data[i].name;
				if (isNaN(edit.data[i].real)) isCodeNum = false;
			}
			//alert(codes);
			var strCodes = "'"+codes.replaceAll(",","','")+"'";
			if (isCodeNum) return "map(if(ifnumber(value()),list("+codes+"),list("+strCodes+")),list("+(dispNum?disps:("'"+disps.replaceAll(",","','")+"'"))+"))";
			else return "map(list("+strCodes+"),list("+(dispNum?disps:("'"+disps.replaceAll(",","','")+"'"))+"))";
	...
	obj.rpxUtils = {
		...
		,getDqlSegments : function(conf) {
			...
			for (var i=0; i<conf.tops.length; i++)
			{
				...
				if (topi.relas.length>0) {
					for (var j=0; j<topi.relas.length; j++) {
						var jObj = dqlQuery.confUtils.getInfosObj(topi.relas[j]);//2020.7.8表头查不到上级(topi.relas[j]);//topi.src
						var tj = getItemByAttr(sgts.tables,jObj.firstTable,"table");
						if (tj == null) {
							tj = {table :jObj.firstTable,alias : 'T'+(sgts.tables.length+1),bys : [],where : '',joinType : 'JOIN'};
							sgts.tables.push(tj);
			...
			for (var i=0; i<conf.fields.length; i++)
			{
				...
				var iObj = dqlQuery.confUtils.getInfosObj(fieldi.src, fieldi.calc);//2020.02.20上级的上级问题, || fieldi.autoSrc != fieldi.src
...
//			return dql;
		...
		notSimple : function(srcsql){
			...
			var fromPos = srcsql.indexOf(" FROM ");
			if (fromPos == -1) return true;
			var fields = $.trim(srcsql.substring(0,fromPos).replace("SELECT","").replace("FROM","")).split(",");
			var hasAlias = false;
			for (var i=0; i<fields.length; i++) {
				if ($.trim(fields[i]).indexOf(" ")>=0||$.trim(fields[i]).indexOf(" AS ")>=0) hasAlias = true;
			}			
			var hasLimit = srcsql.indexOf(" LIMIT ") >= 0;
			...
			return hasLimit || hasAlias || hasGroupby || hasWhere || hasHaving || hasJoin || hasUnion || hasInnerSelection;
		}
				...
				try {
					...
					if (f._parentType != 'field') {
						...
						for (var i=0; i<usedTables.length; i++) {
							...
							if (infoss.length>0) {
								var posii = infoss.indexOf(f.src);
								if (posii==-1) {
									posii = infoss.indexOf(f.autoSrc);
								} 
								if (posii == -1) posii = 0;
								if (f.relas.indexOf(infoss[posii])==-1) f.relas.push(infoss[posii]); 
